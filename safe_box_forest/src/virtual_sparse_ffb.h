...
inline std::optional<VirtualSeedCell> virtual_seed_cell_at_depth(
    ...
    for (int depth = 0; depth < target_depth; ++depth) {
        ...
        if (dim < 0 || dim >= static_cast<int>(intervals.size()) || dim >= tree_seed.size()) {
            return std::nullopt;
        }
        const double split_value = policy.choose_split_value(intervals[static_cast<std::size_t>(dim)]);
