...
- D18 prewarm baseline used LinkIAABB for the first executable mechanism baseline under the leaf-only FK rule: 524287 nodes, 524287 evidence records, 262144 direct leaf materializations, 20.17 s first prewarm wall time, 1.505 s reopen pass, and 262144 reused endpoint evidence records.
...
- E2 warm d18 baseline artifact: `safe_box_forest/outputs/paper/rbf_only/e2_warm_d18_baseline.json`, `ok=true`. Both warm rows build certified forests: Shelf+IIWA warm reports 128 boxes / 4 segment edges with wall time 1.812 s, 3420 external evidence hits, and 1809 active endpoint-cache hits; random-IIWA easy warm reports 128 boxes / 1 segment edge with wall time 0.1727 s, 3002 external evidence hits, and 664 active endpoint-cache hits.
- E3 Shelf+IIWA artifact: `safe_box_forest/outputs/paper/rbf_only/e3_shelf_iiwa_main.json`, `ok=true` at seed 0. Cold and warm-d18 rows both pass all five Marcucci query pairs under strict audit with 256 certified boxes and 3 segment edges; the warm row reports 1.588 s wall time, 7201 external evidence hits, 3946 active endpoint-cache hits, and zero repairs.
