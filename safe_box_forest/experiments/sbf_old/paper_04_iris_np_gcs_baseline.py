...
def solve_regions_gcs(q_start: np.ndarray, q_goal: np.ndarray, regions: list[Any], *, seed: int, checker: Any,
    ...
    if unsafe:
        ...
        return {"success": True, "time_s": time.perf_counter() - t0, "path_length": path_length(repaired), "raw_path_length": raw_length, "regions": len(regions), "edges": len([edge for edge in gcs.gcs.Edges()]), "waypoints_count": int(repaired.shape[0]), "raw_waypoints_count": int(path.shape[0]), "collision_checked": True, "collision_free": True, "unsafe_segments": 0, "gcs_unsafe_segments": len(unsafe), "repaired_segments": repaired_segments, "repair_time_s": repair_s, "note": "GCS path repaired by local collision-checked RRT-Connect"}
    return {"success": True, "time_s": time.perf_counter() - t0, "path_length": raw_length, "regions": len(regions), "edges": len([edge for edge in gcs.gcs.Edges()]), "waypoints_count": int(path.shape[0]), "collision_checked": True, "collision_free": True, "unsafe_segments": 0}
