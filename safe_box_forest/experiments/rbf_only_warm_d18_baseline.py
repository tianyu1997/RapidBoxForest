...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the RBF-only E2 warm d18 baseline.")
...
def make_shelf_case() -> dict[str, Any]:
    query = sbf.make_combined_queries()[0]
    return {
...
def make_random_case(args: argparse.Namespace) -> dict[str, Any]:
    robot_name = "iiwa"
    scene = make_random_scene(robot_name, args.random_difficulty, int(args.seed_base) + 1009 * int(args.random_seed), scene_profile="balanced")
    return {
        "scene": f"random_{robot_name}_{args.random_difficulty}",
        "robot_name": robot_name,
        "robot": make_robot(robot_name),
        "obstacles": scene.obstacles,
        "coverage_seeds": [scene.start, scene.goal],
        "queries": [argparse.Namespace(label="start->goal", start=scene.start, goal=scene.goal)],
    }
...
def run_row(args: argparse.Namespace, case: dict[str, Any], mode: str, seed: int) -> dict[str, Any]:
    ...
    print(
        f"[e2] done scene={case['scene']} mode={mode} ok={row['ok']} "
        f"planning_s={planning_s:.3f} maintenance_s={row['build']['maintenance_s']:.3f} "
...
def main() -> int:
    args = parse_args()
    cases = [make_shelf_case(), make_random_case(args)]
    rows: list[dict[str, Any]] = []
    for index, case in enumerate(cases):
        rows.append(run_row(args, case, "cold", seed=index))
        rows.append(run_row(args, case, "warm_d18", seed=index))
    payload = {
    ...
    print(f"[e2] wrote {args.out_json} ok={payload['ok']}", flush=True)
    return 0
