...
public:
    ...
    int  reserve_count() const noexcept { return reserve_count_; }
...
public:
    ...
    void reserve_from_bounds(const double lo[3], const double hi[3]) {
        int bx = brick_of(to_cell(hi[0], 0)) - brick_of(to_cell(lo[0], 0)) + 1;
        int by = brick_of(to_cell(hi[1], 1)) - brick_of(to_cell(lo[1], 1)) + 1;
        int bz = brick_of(to_cell(hi[2], 2)) - brick_of(to_cell(lo[2], 2)) + 1;
        if (bx < 1) bx = 1;
        if (by < 1) by = 1;
        if (bz < 1) bz = 1;
    ...
    void fill_aabb(const float* aabb) {
        ...
        for (int bz = bz0; bz <= bz1; ++bz) {
            ...
            for (int by = by0; by <= by1; ++by) {
                ...
                for (int i = 0; i < n_xb; ++i) local_bricks[i].clear();
                ...
                for (int cz = cz_s; cz <= cz_e; ++cz) {
                    ...
                    for (int cy = cy_s; cy <= cy_e; ++cy) {
                        ...
                        ++range_write_count_;
                        local_range_write_count_ += or_range_x_local(local_bricks, bx0, cx0, cx1, ly, lz);
                    }
                }
            ...
            for (int by = by0; by <= by1; ++by) {
                ...
                for (int cz = cz_s; cz <= cz_e; ++cz) {
                    ...
                    for (int cy = cy_s; cy <= cy_e; ++cy) {
                        ...
                        ++range_write_count_;
                        local_range_write_count_ += or_range_x_local(local_bricks, bx0, cxs, cxe, ly, lz);
                    }
                }
    ...
    int grow_count() const noexcept { return bricks_.grow_count(); }
    int reserve_count() const noexcept { return bricks_.reserve_count(); }
    int capacity() const noexcept { return bricks_.capacity(); }
    int64_t range_write_count() const noexcept { return range_write_count_; }
    int64_t local_range_write_count() const noexcept { return local_range_write_count_; }
    int64_t brick_write_count() const noexcept { return brick_write_count_; }
    int64_t fallback_range_write_count() const noexcept { return fallback_range_write_count_; }
    ...
    void clear() {
        bricks_.clear();
        range_write_count_ = 0;
        local_range_write_count_ = 0;
        brick_write_count_ = 0;
        fallback_range_write_count_ = 0;
    }
...
private:
    ...
    BrickMap bricks_;
    int64_t range_write_count_ = 0;
    int64_t local_range_write_count_ = 0;
    int64_t brick_write_count_ = 0;
    int64_t fallback_range_write_count_ = 0;
    ...
    static int or_range_x_local(BitBrick* local_bricks,
                                int bx0,
                                int cx_start,
                                int cx_end,
                                int ly,
