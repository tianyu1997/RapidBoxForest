#pragma once
/// @file log.h
/// @brief Lightweight logging macros for RBFPlanningForest.
///
/// Usage:
...
///
/// Control:
///   rbf::set_log_level(rbf::LogLevel::SILENT);   // compile-time-style
///   SBF_LOG_LEVEL=0 ./my_binary                   // env override
///   SBF_LOG_FILE=/tmp/build.log ./my_binary       // redirect log to file
///
/// At program startup call `rbf::init_log_from_env()` to honour both env
...
#define SBF_LOG(level, fmt, ...)                                               \
    do {                                                                       \
        if (static_cast<int>(::rbf::global_log_level()) >=                     \
            static_cast<int>(level))                                           \
            std::fprintf(::rbf::global_log_sink(),                             \
                         fmt "\n", ##__VA_ARGS__);                             \
    } while (0)

#define SBF_ERROR(fmt, ...) SBF_LOG(::rbf::LogLevel::ERROR, fmt, ##__VA_ARGS__)
#define SBF_WARN(fmt, ...)  SBF_LOG(::rbf::LogLevel::WARN,  fmt, ##__VA_ARGS__)
#define SBF_INFO(fmt, ...)  SBF_LOG(::rbf::LogLevel::INFO,  fmt, ##__VA_ARGS__)
#define SBF_DEBUG(fmt, ...) SBF_LOG(::rbf::LogLevel::DEBUG, fmt, ##__VA_ARGS__)
#define SBF_TRACE(fmt, ...) SBF_LOG(::rbf::LogLevel::TRACE, fmt, ##__VA_ARGS__)
