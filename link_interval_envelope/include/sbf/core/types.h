#pragma once
/// @file types.h
/// @brief Core data types and constants for RBFPlanningForest v6.
///
...
/// @brief Safety provenance attached to a C-space box.
///
/// `CertifiedFree` denotes boxes validated by certified endpoint envelopes.
/// `ProvisionalFree` denotes boxes accepted under a heuristic/coverage mode and
/// should be audited before being treated as strict certificates.
enum class BoxSafetyStatus : uint8_t {
    Unknown = 0,
    CertifiedFree = 1,
    ProvisionalFree = 2,
    Occupied = 3,
...
struct BoxNode {
    ...
    bool strict_audit_required = false;         ///< True when provisional evidence needs strict audit.
