# Improve.md Implementation Matrix
...
- Sidecar C-LECT mechanisms and audits live under `improve_workspace/`.
- A small number of production scaffolds/diagnostics have been added where
  source anchors were needed, but the full production C-LECT profile is not
  integrated as the default planner.
...
The machine-readable requirement audit is generated by:

```text
python3 improve_workspace/tools/audit_improve_requirements.py
```
...
Production integration readiness is generated by:

```text
python3 improve_workspace/tools/audit_production_integration_readiness.py
```
