...
\input{../sbf_old/generated/tab_tro_link_envelope_widthwise_ifk_s1.tex}
...
Experiment~3 reports the real endpoint-only Shelf+IIWA p18 persisted cache used
by the warm-build study and the immutable read snapshot used by the planner read
path. The cache is built once by an 8-thread p18 prewarm that materializes only
the depth-18 leaf endpoint envelopes and then lets the database propagate child
hulls upward through the remaining 17 levels. The rebuilt mutable namespace
stores 524{,}287 nodes and 524{,}287 endpoint-envelope records, occupies
443.07~MB on disk, and touches 262{,}144 depth-18 leaves during the one-time
prewarm. Publishing the immutable read snapshot adds a 405.45~MB mmap-only
artifact. Table~III reports the prewarm and snapshot publication costs together
with the planner read-lane microbenchmark on that snapshot. Strict verification
remains available as an offline audit, but it is no longer folded into the timed
open row because it measures full consistency checking rather than planner
read-path latency.

\begin{table}[t]
  \caption{LECT endpoint-only persisted-cache microbenchmark on the real
  Shelf+IIWA p18 cache. The prewarm and snapshot-build rows are one-time
  publication steps; the remaining rows open and query the immutable read
  snapshot, reporting average per operation using the most readable unit for
  each row.}
  ...
  \begin{tabular}{@{}lrr@{}}
    ...
    Operation & Ops & Avg. \\
    \midrule
    P18 prewarm (8 threads) & 1 & 72.365 s \\
    Snapshot build & 1 & 242.606 ms \\
    Snapshot open read-only & 1 & 0.026 ms \\
    Node box & 20000 & 0.473 $\mu$s \\
    Exact box lookup & 20000 & 0.409 $\mu$s \\
    Exact endpoint lookup & 20000 & 0.707 $\mu$s \\
    Range query & 20000 & 41.473 $\mu$s \\
    Evidence read & 20000 & 0.190 $\mu$s \\
    \bottomrule
  \end{tabular}
\end{table}

\Cref{tab:exp3_lect_disk_microbench} shows that the endpoint-only persisted
cache keeps the planner read path lightweight after the one-time p18 prewarm and
snapshot publication. On the real 524k-node Shelf+IIWA artifact, opening the
immutable read snapshot costs 0.026~ms, point evidence read is
0.190~$\mu$s, exact box lookup is 0.409~$\mu$s, and the fused exact-endpoint
lookup used by the warm external-evidence path is 0.707~$\mu$s. The wider
intersecting range-query stage remains tens of microseconds at
41.473~$\mu$s, still far below rebuilding the cache. The heavier offline steps
are the one-time p18 prewarm at 72.365~s and snapshot publication at
242.606~ms; both are paid once per published cache generation rather than once
per planner open.
