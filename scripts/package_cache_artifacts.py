...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--archive-format",
        choices=("tar.gz", "tar.zst"),
        default="tar.gz",
        help="Archive format for generated cache bundles. Use tar.zst for large public cache artifacts.",
    )
    parser.add_argument(
        "--gzip-compresslevel",
        type=int,
    ...
    parser.add_argument(
        ...
        choices=range(1, 20),
        metavar="1-19",
        help="Zstandard compression level used with --archive-format tar.zst.",
    )
...
def create_tar_zst(source_dir: Path, archive_path: Path, *, arc_prefix: Path, level: int) -> None:
    zstd = shutil.which("zstd")
    if not zstd:
        raise RuntimeError("zstd executable not found; install zstd or use --archive-format tar.gz")
    command = [zstd, "--no-progress", "-q", f"-{level}", "-T0", "-o", str(archive_path), "-"]
    ...
    try:
        ...
        raise RuntimeError("zstd process closed before tar stream completed") from exc
...
def create_archive(
    ...
    *,
    ...
    if archive_format == "tar.gz":
        ...
        create_tar_zst(source_dir, archive_path, arc_prefix=arc_prefix, level=zstd_level)
    else:
        raise ValueError(f"unsupported archive format: {archive_format}")


def artifact_archive_name(artifact: dict[str, Any], *, archive_format: str) -> str:
    artifact_id = str(artifact["id"])
    return f"{artifact_id}.{archive_format}"
...
def main() -> int:
    ...
    for artifact in artifacts:
        ...
        if archive_metadata is None:
            archive_name = artifact_archive_name(artifact, archive_format=str(args.archive_format))
    ...
    output_manifest = {
        ...
        "description": "Filled local LECT cache artifact manifest generated by scripts/package_cache_artifacts.py.",
        "artifacts": packaged,
    }
