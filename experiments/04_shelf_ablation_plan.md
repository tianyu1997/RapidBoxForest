...
4. 消融：no LECT cache、CritSample、AABB、KDOP、single thread、round-robin split。
...
4. AABB 快但过滤弱；KDOP 介于 AABB 和 SupportHull 之间。
5. Single thread 拖慢 build/prewarm；round-robin 增加 tree/refine 成本。

## 初始脚本

`exp04_shelf_ablation/run_shelf_ablation.py` 先生成完整消融矩阵，并执行旧 runner 能表达的配置；no-cache、true round-robin 和完整 warm AAFKVolumeMin cache hook 标记为后续 native hook。
